...
<%
	//[1] AsideForUser.jsp에서 메뉴추가를 클릭하면 get방식으로 이동
...
<script type="text/javascript">
	$(function(){
		$('#resInfo > td').click(function(){
			var tdResName=$(this).text();
			var tdResNo=$(this).prev('#tdResNo').text();
...
<main class="mainDiv">
	<div class="mainDivGr5 center">
					...
					<%if(list==null || list.isEmpty()){ %>
				  	...
				  	<%}else{
						for(int i=0;i<list.size();i++){			  
							...
							%>		
							<tr id="resInfo" style="text-align:center">
								<td id="tdResNo" width="100px"><%=resNo %></td>
								<td id="tdResName" width="380px"><%=resName %>
								</td>
							<tr>
				  		<%}//for%>
					<%}//if %>
		...
		<form id="frmMenu" name="frmMenu" method="post" action="<%=request.getContextPath() %>/menu/menuAdd_ok.jsp"
			...
			<input type="submit" value="등록 신청" id="joinBtn"
